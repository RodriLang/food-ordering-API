...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (foodVenueRepository.count() == 0) {
            ...
            // Tables
            Table t1 = Table.builder().number(1).capacity(4).status(TableStatus.AVAILABLE).foodVenue(v1).build();
            Table t2 = Table.builder().number(2).capacity(2).status(TableStatus.AVAILABLE).foodVenue(v2).build();
            Table t3 = Table.builder().number(3).capacity(6).status(TableStatus.AVAILABLE).foodVenue(v3).build();
